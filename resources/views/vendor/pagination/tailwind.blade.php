{{--
    Published from the framework (vendor:publish --tag=laravel-pagination)
    and modified for accessibility:
    - Visible focus ring (stock was focus:ring ring-gray-300 = 1.47:1 on
      white, effectively invisible). focus:z-10 lifts the focused link so
      the -ml-px overlap doesn't hide the ring.
        ...
        <div class="hidden sm:flex-1 sm:flex sm:gap-2 sm:items-center sm:justify-between">
            ...
            <div>
                <span class="inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    <a href="{{ $url }}" class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:z-10 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150 hover:bg-gray-100" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
                    ...
                    {{-- Next Page Link --}}
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md leading-5 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:z-10 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150" aria-label="{{ __('pagination.next') }}">
                            <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
